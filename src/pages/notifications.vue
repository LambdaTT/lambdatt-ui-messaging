    ...
    <div ref="scrollTargetRef">
      <q-infinite-scroll @load="fetchNotifications" :offset="250">
        <div
          ...
          class="text-grey-8 text-justify q-gutter-y-md"
        >
          <div v-for="(notification, index) in notifications" :key="index">
            <q-card
              class="round-sm border-left-5-solid q-pa-md"
              clickable
              @click="openNotification(notification)"
              :style="`border-color: ${
                notification.important == 'Y' ? '#F2C037' : 'transparent'
              }`"
            >
              <!-- Header -->
        ...
        <div v-else>
          <div class="column q-pa-md flex-center text-center">
            <div class="q-mb-sm">
              <q-icon name="fas fa-bell" size="60px" color="grey-6"
                ><q-badge floating color="grey-8">0</q-badge></q-icon
              >
            </div>
            <div class="text-bold q-mb-sm" style="font-size: 18px">
              Nenhuma notificação encontrada!
            </div>
            <div>
              Assim que receber sua primeira notificação,<br />ela aparecerá
              aqui nesta página
            </div>
          </div>
        </div>
        <template
          v-if="!stopRequests && notifications.length > 0"
          v-slot:loading
        >
  ...
  methods: {
      ...
      try {
        ...
        if (response && response.data) {
          return response.data.map((notify) => ({
            ...
            authorImg: notify.author_avatar
              ? notify.author_avatar == "system"
                ? "/resources/img/system-icon.jpg"
                : notify.author_avatar
    ...
    async markAsRead(item) {
      try {
        ...
        await this.$getService("toolcase/http").put(
          `${ENDPOINTS.NOTIFICATIONS.NOTIFICATION}/mark-as-read/${item.ds_key}`,
        );
        item.read = "Y";
      } catch (error) {
        this.$getService("toolcase/utils").notifyError(error);
        console.error(
          "An error occurred while attempting to update the object's data.",
          error,
        );
      }
    },
    ...
    async remove(key) {
      ...
      try {
        ...
        this.$getService("toolcase/utils").notifyError(error);
        console.error(
          "An error occurred while attempting to delete the object.",
          error,
        );
      } finally {
